...
1. Xcode を起動します。
2. `File`（ファイル） > `Open...`（開く）から、このリポジトリの `MenuBarNetworkMonitor.xcodeproj` を開きます。
3. スキームで `Network Monitor` を選択します。
4. 実行先に `My Mac` を選択します。
5. `Product`（製品） > `Build`（ビルド）を実行します。

### Xcode で通常の macOS アプリとして起動する方法

1. Xcode で `MenuBarNetworkMonitor.xcodeproj` を開きます。
2. スキームで `Network Monitor` を選択します。
3. Run 先に `My Mac` を選択します。
4. `Product`（製品） > `Run`（実行）、または `Cmd + R` で起動します。
5. 起動後、Dock には表示されず、メニューバーに `Network Monitor` の項目が表示されます。

### Scheme / Build Settings

`MenuBarNetworkMonitor.xcodeproj` には、通常の macOS App として起動・Archive するための `Network Monitor` スキームを含めています。

- Target: `Network Monitor`（macOS App）
- Run destination: `My Mac`
- Bundle Identifier: `com.example.MenuBarNetworkMonitor`
- App Category: `public.app-category.utilities`
- Deployment Target: macOS 13.0
- Debug / Release: どちらもビルド対象
- Archive: Release 構成で作成
...
通常の macOS アプリとして利用する場合は、以下の流れで `.app` を書き出して `/Applications` に配置します。
...
### Xcode で Archive する方法

1. Xcode で `MenuBarNetworkMonitor.xcodeproj` を開きます。
2. スキームで `Network Monitor` を選択します。
3. `Product`（製品） > `Scheme`（スキーム） > `Edit Scheme...`（スキームを編集）を開きます。
4. `Run` > `Build Configuration` が通常起動確認用は `Debug`、配布前確認用は `Release` に変更できることを確認します。
5. `Archive` > `Build Configuration` が `Release` になっていることを確認します。
6. `Any Mac` または `My Mac` を選択します。
7. `Product`（製品） > `Archive`（アーカイブ）を実行します。
8. Organizer が開き、`Network Monitor` の Archive が表示されることを確認します。

### `.app` として書き出す方法

1. Organizer で作成した Archive を選択します。
2. `Distribute App`（Appを配布）を押します。
3. 社内利用や個人利用では `Custom`（カスタム）または `Copy App`（Appをコピー）を選択します。
4. 署名設定を確認して書き出します。
5. 書き出し先に `Network Monitor.app` が作成されます。

### `/Applications` に配置する方法

Finder で書き出した `Network Monitor.app` を `/Applications` フォルダへドラッグ&ドロップします。コマンドラインで配置する場合は、書き出し先に合わせて以下を実行します。

```bash
cp -R "Network Monitor.app" /Applications/
```

### Finder または Launchpad から起動する方法

`/Applications/Network Monitor.app` を Finder でダブルクリックするか、Launchpad から `Network Monitor` を選択します。起動後は Dock には表示されず、メニューバーに常駐します。

### 初回起動時に警告が出た場合の対応

未署名または開発者署名のアプリとして書き出した場合、初回起動時に macOS の Gatekeeper 警告が表示されることがあります。信頼できる自分のビルドであることを確認したうえで、以下のいずれかで起動してください。

- Finder で `Network Monitor.app` を右クリックし、`開く` を選択して確認ダイアログで再度 `開く` を押します。
- `システム設定` > `プライバシーとセキュリティ` で、ブロックされたアプリの `このまま開く` を選択します。
